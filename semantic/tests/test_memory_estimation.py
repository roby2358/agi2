...
class TestMemoryEstimation(unittest.TestCase):
    ...
    def test_mixed_precision_memory_savings(self):
        ...
        print(
            f"Memory savings: {memory_fp32['total_estimated_gb'] - memory_fp16['total_estimated_gb']:.2f} GB"
        )

        # Float16 should use less memory
        self.assertLess(
            memory_fp16["total_estimated_gb"], memory_fp32["total_estimated_gb"]
        )
    ...
    def test_batch_size_scaling(self):
        ...
        for batch_size in batch_sizes:
            memory = estimate_gpu_memory_requirements(
                batch_size=batch_size, **base_params
            )
