...
def estimate_gpu_memory_requirements(
    ...
    if include_activations:
        ...
        activation_memory_gb = (batch_size * seq_len * model_embd * model_layer * 3) / (
            1024**3
        )
...
def load_checkpoint(
    model: nn.Module, optimizer, filepath: str, device: Optional[torch.device] = None
) -> Dict:
    """
...
def get_device_info() -> Dict[str, str]:
    ...
    info = {
        ...
        "current_device": (
            str(torch.cuda.current_device()) if torch.cuda.is_available() else "N/A"
        ),
    }
