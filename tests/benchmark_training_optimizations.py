...
def benchmark_causal_mask_caching():
    ...
    for (cached_seq_len, cached_device), mask in model._causal_mask_cache.items():
        print(
            f"  Cached mask: seq_len={cached_seq_len}, device={cached_device}, shape={mask.shape}"
        )
...
def benchmark_memory_usage():
    ...
    print(
        f"Baseline memory: {baseline_allocated:.1f}MB allocated, {baseline_reserved:.1f}MB reserved"
    )
    ...
    print(
        f"After caching: {cached_allocated:.1f}MB allocated, {cached_reserved:.1f}MB reserved"
    )
    ...
    print(
        f"After clearing cache: {cleared_allocated:.1f}MB allocated, {cleared_reserved:.1f}MB reserved"
    )
